...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs> &observations) {
  ...
  for (auto &observation : observations) {
    ...
    double dl_prev = 10000.0;
    int id = -1;
    for (auto &prediction : predicted) {
      auto dx = obs_x - prediction.x;
      auto dy = obs_y - prediction.y;
      auto dl = sqrt(pow(dx, 2) + pow(dy, 2));
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
  ...
  for (auto &particle : particles) {
    ...
    for (auto &landmark : map_landmarks.landmark_list) {
      auto l_id = landmark.id_i;
      double l_x = landmark.x_f;
      double l_y = landmark.y_f;

      auto dx = l_x - x;
      auto dy = l_y - y;
      auto dis = sqrt(pow(dx, 2) + pow(dy, 2));
      if (dis <= sensor_range) {
//        l_x = x * cos(x) + y * sin(theta) + x;
//        l_y = y * cos(x) - x * sin(theta) + y;
    ...
    for (auto &obs : observations){
      auto l_x = cos(theta)*obs.x - sin(theta) * obs.y + x;
      auto l_y = sin(theta)*obs.x + cos(theta) * obs.y + y;
      observations_t.push_back(LandmarkObs{obs.id, l_x, l_y});
    }
    ...
    for (auto &obs : observations_t){
      ...
      new_w = new_w * exp(-0.5 * (pow(dx, 2.0) * std_landmark[0] + pow(dy, 2.0) * std_landmark[1])) /
              sqrt(2.0 * M_PI * std_landmark[0] * std_landmark[1]);
...
void ParticleFilter::resample() {
  ...
  for (auto &particle : particles){
    ...
    particles_r.push_back(particles[index(gen)]);
  }
